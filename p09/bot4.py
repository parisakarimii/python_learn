...
bot = Bot("500767679:iurWx1uJSmU5WsHEIx8YnaQAcp3CBUOc9DLwAAPI")
questions = [
    {
...
def send_statistics(user_id):
    total_questions = len(info[user_id]["shown_q"])
    correct_answers = info[user_id]["score"]
    message = f"شما {correct_answers} سوال درست از {total_questions} سوال جواب داده‌اید!"
    data = {
        "chat_id": user_id,
        "text": message
    }
    bot.send_message(json.dumps(data).encode())
